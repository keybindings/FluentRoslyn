...
- **Publishing is done.** `FluentRoslyn` `0.1.0-preview.9` is live on nuget.org —
...
| 40 | **Static member access as a receiver** | Feature | Low | Med | Pulled by the R2-01 fix. `EqualityComparer<T>.Default` is a static *property*, and no built call can root on it: `CallStatic` reaches static methods only, and `InvokeRaw` needs an `IReference`. The value-objects generator hoists the receiver into a `private static readonly` field — a decent pattern (the field *is* a reference, so the calls through it stay built) but a workaround. Build a `Type.Member` reference form if another generator hits it. |
...
open is no longer coverage but those findings; 26 are fixed and the rest live there, with
